...
async function checkStatus (github, file) {
  let {status} = await github.get(`/repos/${github.repoName}/contents/${file.path}?branch=${github.branchName}`)
    .catch(err => {
...
async function sameOnBranch (github, file) {
  ...
  const {data} = await github.get(`/repos/${github.targetRepo}/contents/${file.path}?branch=${github.branchName}`)
    .catch(err => {
      if (err) {
...
async function commitFile (github, file) {
  ...
  if (!file.newFile) {
    commit.sha = await getCurrentSha(github, file.path)
  }

  await github.put(`/repos/${github.targetRepo}/contents/${file.path}?branch=${github.branchName}`, commit)
    .catch(err => {
      if (err) {
...
async function bunchFiles (github, filesToCheck, opts) {
  ...
  } = await github.get(`/repos/${github.repoName}/branches/${github.defaultBranch}`)
    .catch(err => {
      if (err) {
...
async function getCurrentSha (github, filename) {
  const {data: {sha: currentSha}} = await github.get(`/repos/${github.targetRepo}/contents/${filename}?branch=${github.branchName}`)
    .catch(err => {
      if (err) {}
