...
function checkBugs (github, pkg, notesForUser) {
  const validBugString = `https://github.com/${github.repoName}/issues`
  if (!pkg.bugs || pkg.bugs === validBugString.replace('https', 'http') || pkg.bugs.url === validBugString.replace('https', 'http')) {
    pkg.bugs = {
      'url': `https://github.com/${github.repoName}/issues`
    }
  } else if ((pkg.bugs.url && pkg.bugs.url !== validBugString) || pkg.bugs !== validBugString) {
...
async function lint (github, pkg) {
  ...
  checkBugs(github, pkg, notesForUser)
